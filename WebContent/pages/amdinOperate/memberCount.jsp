...
<%
	...
	List<User> users = ServiceFactory.createUserService().getAllUsers();
%>
...
<div class="row clearfix">
	...
	<label style="text-align:center;color:red;font-size: 18px">会员总数：<%=users.size() %></label>
			    ...
			    <table class="table table-hover">
			      ...
			      <tbody>
			 		<%
						...
						int otherNum = users.size() - Integer.parseInt(divideNum)*8;
			        ...
			        <tr>
			        	  <td style="text-align: center;"><label><%=user.getEmail()%></label></td>
			        	  <td style="text-align: center;"><label><%=user.getUserPassword()%></label></td>
				          <td style="text-align: center;">
							<a type="button" class="btn btn-primary btn-xs" href="/OnlineFlowerShop/DeleteUser?divideNum=<%=divideNum %>&item=<%=user.getUserId()%>">删除</a>
						  </td>
	...
	<div class="col-md-4 column" style="float: right;">
		<a class="btn btn-info" id="before" href="/OnlineFlowerShop/pages/amdinOperate/memberCount.jsp?divideNum=<%=Integer.parseInt(divideNum)-1%>">上一页</a>
		<a class="btn btn-info" id="next" href="/OnlineFlowerShop/pages/amdinOperate/memberCount.jsp?divideNum=<%=Integer.parseInt(divideNum)+1%>">下一页</a>
	</div>
</div>
